...
-- MAGIC
-- MAGIC Predictive Optimization can be enabled at the account level, or on catalogs and schemas, as long as the user enabling this feature has the correct permission level. The feature is inherited by all lower-level objects, but it can be enabled/disabled on those objects, as needed.
-- MAGIC
-- MAGIC To check whether Predictive Optimization is enabled on a table, run **`DESCRIBE EXTENDED <table name>`**:
-- MAGIC
-- MAGIC
